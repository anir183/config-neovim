...
NMAP("<leader>op", function()
	...
	if options_file == nil then
		local default_options_file_path = vim.fn.stdpath("config")
			.. "/lua/base/default_opts.lua"
			...
			vim.ui.input({ prompt = "Enter tab length: " }, function(tab_len)
				...
				vim.cmd(
					"silent! %s/\\(^\\s*\\)\\@<="
						...
						.. "/	/g"
				) -- substitute instead of retab as the command also replaces inline spaces to tabs.  NOTE : regex from https://stackoverflow.com/a/35050756
				...
				if indentation_style == "spaces" then
					...
					vim.cmd(
						"silent! %s/\\(^\\s*\\)\\@<=	/"
							.. (" "):rep(vim.opt.tabstop._value)
							.. "/g"
					) -- substitute instead of retab as the command also replaces inline tabs with spaces
...
end, { desc = "core/custom: toggle between moving text and moving cursor" })
...
NMAP(
	...
	{
		desc = "core/custom: substitute standalone occurences of inputted word (not string)",
	}
)
NMAP(
	"<leader>SS",
	...
	{
		desc = "core/custom: substitute all occurences of inputted string (even as a substring)",
	}
)
